...
function mostrar(id)
{
	$.post("../ajax/reporte.php?op=mostrar",{id : id}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);

		//lado derecho son variabls de la bd
		$("#carrito_id").val(data.carrito_id);
...
{
	bootbox.confirm("¿Está Seguro de eliminar esta CLIENTE?", function(result){
		if(result)
        {
        	$.post("../ajax/reporte.php?op=eliminar", {id : id}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
